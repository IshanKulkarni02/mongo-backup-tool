...
// TestRunBackupConcurrentCallsDoNotLoseIndexEntries is the regression
// test for #170: the TUI's runBackup did a raw store.Load-mutate-
// store.Save with no synchronization, the same unsynchronized-index
// race #65 fixed for desktop's CreateBackup by routing it through
// store.Update (which holds a package-level lock for the whole
// load-mutate-save span). Concurrent runBackup calls could each load a
// stale index, append their own entry, and have the slower one's save
// silently clobber the faster one's already-saved entry: the archive
// stays on disk but permanently vanishes from index.json.
func TestRunBackupConcurrentCallsDoNotLoseIndexEntries(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func(i int) {
			defer wg.Done()
			_, err := runBackup("backup-race-test", uri, "racedb")
			errs[i] = err
		}(i)
	}
	wg.Wait()
